...
class KafkaConfig:
    ...
    consumer_group: str = os.getenv("INVENTORY_CONSUMER_GROUP", "inventory-service")
...
class AppConfig:
    ...
    consumer_id: str = "inventory-service"
    max_retries: int = int(os.getenv("MAX_RETRIES", 3))
    retry_backoff_base: float = float(os.getenv("RETRY_BACKOFF_BASE", 2.0))
    
    # Order service database connection
    order_db_host: str = os.getenv("MYSQL_HOST", "mysql")
    order_db_user: str = os.getenv("MYSQL_USER", "orderuser")
    order_db_password: str = os.getenv("MYSQL_PASSWORD", "orderpass123")
    order_db_name: str = os.getenv("MYSQL_DATABASE", "orderdb")
